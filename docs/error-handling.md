...
| `PersistenceException` | A write cannot be made or was refused: an uninitialised property, no generated identifier returned, an identifier matching more than one row. |
...
All nine are `final`. `EntityException` is the one open class, and it is what to
